...
import { CalendarOutlined, ClockCircleOutlined, LogoutOutlined, ReloadOutlined } from "@ant-design/icons";
import { Alert, Button, DatePicker, Empty, Skeleton, Table, Tag } from "antd";
...
export default function TurnosDashboard({ username }: { username: string }) {
  ...
  const columns = useMemo<TableColumnsType<Turno>>(
    () => [
      ...
      { title: "ESTADO", key: "estado", render: () => <Tag color="success">Disponible</Tag> },
      { title: "", key: "action", align: "right", render: () => <Button type="primary" shape="round" href="https://neptunia.brio.club/" target="_blank">Reservar</Button> },
    ],
    [],
  );
    ...
    <main className="dashboard">
      ...
      <section className="results">
        <div className="results-head">
          <div><p className="eyebrow"><ClockCircleOutlined /> TURNOS DISPONIBLES</p><h2>{prettyDate(fecha)}</h2><p className="muted">{loading ? "Consultando canchas…" : `${turnos.length} turnos en ${courts} canchas`}{updated && !loading ? ` · actualizado ${updated.toLocaleTimeString("es-AR", { hour: "2-digit", minute: "2-digit" })}` : ""}</p></div>
